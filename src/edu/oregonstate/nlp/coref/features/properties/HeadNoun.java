...
public Object produceValue(Annotation np, Document doc)
{
   ...
   */
  String txt = doc.getAnnotString(np);
  if(txt.matches("\\S+"))
    return new Annotation(np.getId(), np.getStartOffset(), np.getEndOffset(), "HeadNoun",
        new HashMap<String, String>());
  AnnotationSet posAnnots = doc.getAnnotationSet(Constants.POS);
  AnnotationSet parse = doc.getAnnotationSet(Constants.PARSE);
  Annotation value;
  String[] prepPOS = { "IN", "TO", "WP", "WP$" };
  String[] separator = { ",", ":", ";" };
  ...
  for (int i = 1; i < ordrdToks.size(); i++) {
    ...
    if (FeatureUtils.memberArray(curAnnotation.getType(), separator)) {
      ...
      // print = true;
      AnnotationSet contained = parse.getContained(curAnnotation.getEndOffset(), np.getEndOffset()+1);
      boolean end = false;
      if (contained != null) {
    ...
    if ((FeatureUtils.memberArray(curAnnotation.getType(), prepPOS) && !doc.getAnnotText(curAnnotation)
        .equalsIgnoreCase("than"))||curAnnotation.getType().startsWith("W")) {
