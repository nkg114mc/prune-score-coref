...
public String produceValue(Annotation np1, Annotation np2, Document doc, Map<Feature, String> featVector)
{
  ...
  if (maxNP1.compareSpan(np2) == 0||maxNP2.compareSpan(np1) == 0) {
    if (Constructor.createFeature("Prednom").getValue(np1, np2, doc, featVector).equals(COMPATIBLE)) return INCOMPATIBLE;
    if (Constructor.createFeature("RoleAppositive").getValue(np1, np2, doc, featVector).equals(COMPATIBLE)) return INCOMPATIBLE;
