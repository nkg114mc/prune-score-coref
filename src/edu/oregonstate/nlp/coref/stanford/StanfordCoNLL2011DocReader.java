 ...
 * @author Angel Chang
 */
public class StanfordCoNLL2011DocReader {
...
//  private String filepath;
  ...
  public static final Logger logger = Logger.getLogger(StanfordCoNLL2011DocReader.class.getName());

  public StanfordCoNLL2011DocReader(String filepath)
  {
    this(filepath, new Options());
  }

  // read conll files from a directory
  public StanfordCoNLL2011DocReader(String filepath, Options options)
  {
...
/*
  // read a single conll file
  public initForSingleConllFile(String conllfilepath, Options options) {
//    this.filepath = filepath;
	  this.fileList = getFiles(filepath, options.filePattern);
	  this.options = options;
	  curFileIndex = 0;
...
*/
  ...
  public StanfordCoNLL2011DocReader.Document getNextDocument()
  {
    try {
      ...
      StanfordCoNLL2011DocReader.Document next = docIterator.next();
  ...
  public void initIteratorWithSingleConllFile(String conllfilepath)
  {
	//ArrayList<StanfordCoNLL2011DocReader.Document> alldocs = new ArrayList<StanfordCoNLL2011DocReader.Document>();
    ...
    try {
      ...
      if (!conllfile.exists()) { // conll file should exist!
    	  throw new FileNotFoundException(conllfile.getAbsolutePath());
      }
      // read it
      docIterator = new DocumentIterator(conllfile.getAbsolutePath(), options);
      /*
      while ( docIterator.hasNext()) {
          System.out.println("Processed " + docIterator.docCnt + " documents in " + conllfile.getAbsolutePath());
          StanfordCoNLL2011DocReader.Document next = docIterator.next();
          alldocs.add(next);
      }
      docIterator.close();
  ...
  public StanfordCoNLL2011DocReader.Document getNextDocumentFromSingleConllFile()
  {
	  if (docIterator == null) return null; // iterator has not be initialized yet.
	  while ( ! docIterator.hasNext()) {
		  docIterator.close();
		  return null;  // DONE!
	  }
	  StanfordCoNLL2011DocReader.Document next = docIterator.next();
	  System.out.println("Reading document: " + next.getDocumentID());
	  return next;
  }
  
  /*
  public ArrayList<StanfordCoNLL2011DocReader.Document> getAllDocumentFronSingleConllFile(String conllfilepath)
  {
	ArrayList<StanfordCoNLL2011DocReader.Document> alldocs = new ArrayList<StanfordCoNLL2011DocReader.Document>();
  ...
  public static class Document  extends CoNLL2011DocumentReader.Document {
  ...
  /** Helper iterator **/
  private static class DocumentIterator extends AbstractIterator<StanfordCoNLL2011DocReader.Document> implements Closeable {
    ...
    String filename;
    BufferedReader br;
    StanfordCoNLL2011DocReader.Document nextDoc;
    int lineCnt = 0;
    int docCnt = 0;
    ...
    @Override
    public StanfordCoNLL2011DocReader.Document next() {
      if (nextDoc == null) {
        throw new NoSuchElementException("DocumentIterator exhausted.");
      }
      StanfordCoNLL2011DocReader.Document curDoc = nextDoc;
      nextDoc = readNextDocument();
      return curDoc;
    }
    ...
    public void annotateDocument(StanfordCoNLL2011DocReader.Document document)
    {
    ...
    public StanfordCoNLL2011DocReader.Document readNextDocument() {
      try {
        List<String[]> curSentWords = new ArrayList<String[]>();
        StanfordCoNLL2011DocReader.Document document = null;
        for (String line; (line = br.readLine()) != null; ) {
          ...
          if (line.length() != 0) {
            if (line.startsWith(docStart)) {
              ...
              document = new StanfordCoNLL2011DocReader.Document();
  ...
  public static void main(String[] args) throws IOException {
    ...
    StanfordCoNLL2011DocReader reader = new StanfordCoNLL2011DocReader(filepath, options);
    int docCnt = 0;
    int sentCnt = 0;
    int tokenCnt = 0;
