...
//import edu.oregonstate.nlp.coref.uiuc.UIUCConllDocLoader;

public class OntoNotesPreprocessor {
	...
	//private String inputTrainDirPath = null;
	//private String inputDevDirPath = null;
	//private String inputTestDirPath = null;

	private ArrayList<ConllDocument> conllDocList = null;
	private ArrayList<Document> reconDocList = null;
	/** Sometimes, the number of docs is too large, we may only keep their names,
	 *  and load them one after another. The following list is to store the names
	 *  of the two doclist above. */
	...
	public void setCfg(SystemConfig cfg) {
		...
		*/
		...
		HashMap<String, String[]> elSetNames = AnnotationSetting.getPreprocessingElSetNames();
	...
	public void preprocess(File conllFilename) {
		...
		try {
			...
			AnnotationSet annoSet = OntoNotesConllReader.extractConllAnnotationSet(newConllFile.getAbsolutePath(), Constants.CONLL_ANNO);
			...
			int partNum = OntoNotesConllReader.getTotalPartNum(annoSet);
			...
			for (int j = 0; j < partNum; j++) {
				...
				OntoNotesConllReader.writeConllFormatPart(annoSet, partDirFile.getAbsolutePath(), partName, j);
				...
				//OntoNotesConllReader.run(doc, conllSetName);
	...
	private void runScript(String scriptPath, String scriptArg) {
		...
		try {
			Process p = Runtime.getRuntime().exec("bash " + scriptPath + " " + scriptArg);
			p.waitFor(); 
		} catch (IOException e) {
			e.printStackTrace();
