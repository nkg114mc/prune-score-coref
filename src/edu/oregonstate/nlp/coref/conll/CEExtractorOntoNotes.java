...
public class CEExtractorOntoNotes extends CEExtractor{// InternalAnnotator {
...
public boolean isNoun(Annotation a, String text)
{
	...
	// return true;
	if (type.startsWith("NNP") && FeatureUtils.isCapitalized(Utils.getAnnotText(a, text))) return true;
...
public boolean isNP(Annotation an, String text)
{
	try {
		...
		assert an.getStartOffset()!=-1 && an.getEndOffset()!=-1 : 
			"offset -1 " + text + " : " + an;
		// TODO this is done to overcome by problems by & errors in the dataset itself.
		if(an.getStartOffset()==-1 || an.getEndOffset()==-1)
			return false;
		
		if (FeatureUtils.memberArray(type, SyntaxUtils.NPType)&&!FeatureUtils.memberArray(Utils.getAnnotText(an, text),HERE)) 
			return true;
...
*/	//System.out.println("Excluded: "+doc.getAnnotText(a.getStartOffset()-10<0?0:a.getStartOffset()-10,a.getStartOffset())
	...
	String t = a.getType();
	if(t.equalsIgnoreCase("NORP")||t.equalsIgnoreCase("MONEY")||t.equalsIgnoreCase("CARDINAL"))
		return false;
	return true;
}
...
/*
 * This method cleans potential nps by removing leading and trailing junk as well as possesives
 */
public static Annotation trimNP(Annotation np, AnnotationSet parse, String text)
{
 ...
 /*
  // Remove possesive 's as well as some other junk that occurs at end of nps
  for (Annotation p : contained) {
    if ((p.getType().equals("POS") || p.getType().equals("\"") || Utils.getAnnotText(p, text).matches(
        "\\(|\\)|\\\"|\\.|\\,"))
        && p.getEndOffset() == np.getEndOffset()) {
      result = new Annotation(np.getId(), np.getStartOffset(), p.getStartOffset(), "newNP",
...
private static final String[] TITLES = { "mr.", "mr", "messrs", "messrs.", "vice", "ms.", "ms", "mrs.", "mrs", "dr.",
    "dr", "prof.", "prof", "rev", "rev.", "rep.", "rep", "reps.", "reps", "sen.", "sen", "sens.", "sens", "gov.",
    "gov", "gen.", "gen", "maj.", "maj", "col.", "col", "lt.", "lt", "sgt.", "sgt" };
// Strictly company designations
public static String[] CORP_DESIG = { "co.", "co", "cie", "cie.", "cos.", "corp", "corp.", "inc.", "inc", "ltd.",
    "ltd", "ltda.", "ltda", "l.p.", "lp", "Assoc.", "groupe", "grupo", "bros", "bros.", "bancorp", "bancorp.", "sdn",
    "sdn.", "bhd", "bhd.", "plc", "plc.", "s.a.", "s.a", "sa", "M.e.T.A.", "M.e.T.A", "g.m.b.h.", "g.m.b.h", "gmbh",
    "s.p.a.", "s.p.a", "spa", "c.a.", "c.a", "a.g.", "a.g", "ag", "a.b.", "a.b", "ab", "aktiebolaget",
    "aktiengesellschaft", "n.v.", "n.v", "nv", "bv", "b.v.", "b.v", "p.c.", "p.c", "de c.v.", "de c.v", "de cv",
    "b.d.d.p.", "b.d.d.p", "bddp" };
...
public static boolean containsSameHead(AnnotationSet nps, Annotation np, boolean isNP, Document doc)
{
  ...
  for (Annotation n : overlap) {
    ...
    if (head.compareSpan(h) == 0)
      if (!isNP || !containsBreak(n.getStartOffset(), h.getEndOffset(), parse, ne, doc.getText()))
        return true;
      else if (np.compareSpan(n) == 0
          || (n.getStartOffset() == np.getStartOffset() && (h.getEndOffset() == np.getEndOffset() || n.getEndOffset() == head
              .getEndOffset()))) return true;
  }
  return false;
}
...
public static boolean containsBreak(int start, int end, AnnotationSet parse, AnnotationSet ne, String text)
{
  ...
  for (Annotation o : overlap)
    if (o.getType().equals("NP") && (o.getStartOffset() != start || o.getEndOffset() != end)) {
      overlapNps.add(o);
    }
  ...
  for (Annotation o : overlap) {
    ...
    if (o.getType().equals("CC") && !"or".equalsIgnoreCase(Utils.getAnnotText(o, text)) && !ne.coversSpan(o)
        && (overlapNps == null || !overlapNps.coversSpan(o))) return true;
  }
  return false;
}
...
public AnnotationSet getBaseCEs(String annSetName, Document doc)
{
  ...
  // Remove ce that contain PP's or S's
  for (Annotation p : bce) {
    if (!containsPhrase(p, parseAnns, filtered, doc.getText())) {
      filtered.add(p);
      // if(containsConj(p,parseAnns,text))
      // System.out.println("Contains conjunction: "+Utils.getAnnotText(p, text));
    }
    // else
    // System.out.println("Contains phrase: "+Utils.getAnnotText(p, text));
  }
...
public AnnotationSet extractCEs(AnnotationSet bnp, AnnotationSet parse, String annSetName, Document doc)
{
  ...
  // Keep track of the foolowing categories -- basenp's that overlap ne's, other basenps
  ...
  for (Annotation np : baseCEs) {
    ...
    else if (!result.containsSpan(np) && !origNes.coversSpan(np) && !isJunk(np, doc.getText())
  ...
  for (Annotation a : nes) {
    ...
    if (a.getLength() > 0 && !result.containsSpan(a) && !containsSameHead(result, a, true, doc)
        && !isJunk(a, doc.getText()) && !a.getType().equalsIgnoreCase("number")) {
      if (!containsConflict(result, a, doc)) {
    	if(addNE(a,result,bnp, doc))  
    	  result.add(a.getStartOffset(), a.getEndOffset(), "NE");
        // System.out.println("added.");
      }
    }
    // else{
    // System.out.println("-----not added.----"+containsSameHead(result, a, annots, text));
    // }
  }
...
private static void addNP(AnnotationSet result, Annotation np, AnnotationSet nes, AnnotationSet origNes,
    AnnotationSet parse, Document doc)
{
  np = trimNP(np, parse, doc.getText());
  if (!containsSameHead(result, np, true, doc)) {
    ...
    if (!overlapNe.isEmpty()) {
      ...
      if (np.covers(ne)
          || (np.getStartOffset() <= ne.getStartOffset() && doc.getAnnotString(np.getEndOffset(), ne.getEndOffset())
              .matches("\\W+"))) {
        ...
        if (HeadNoun.getValue(ne, doc).compareSpan(HeadNoun.getValue(np, doc)) == 0
        ...
        if (snp != null && HeadNoun.getValue(ne, doc).compareSpan(HeadNoun.getValue(snp, doc)) == 0
            && !containsBreak(snp, parse, origNes, doc.getText())) {
          nes.remove(ne);
        }
      }
    }
...
private static void addNested(Annotation parent, AnnotationSet parse, AnnotationSet result, AnnotationSet origNes,
    AnnotationSet nes, Document doc)
{

  AnnotationSet contained = parse.getContained(parent);
  // System.out.println("Adding nested inside "+FeatureUtils.getText(parent, text));

  for (Annotation par1 : contained) {
    if (par1.getType().equals("NP")) {
      Annotation np = trimNP(par1, parse, doc.getText());
      if (np.getLength() <= 0) {
        // System.out.println("0 annotation from "+Utils.getAnnotText(par1, text));
      }
      else if (parent.properCovers(np) && !result.containsSpan(np) && !origNes.coversSpan(np)
          && !isJunk(np, doc.getText()) && !containsSameHead(result, np, true, doc) /*&& !containsAppositive(np, parse, text)*/) {
        // System.out.println("Found "+FeatureUtils.getText(np, text));
        // add(result, np, text);
        if (!containsConflict(result, np, doc)) {
          addNP(result, np, nes, origNes, parse, doc);
        }
...
private void addNestedNonNPs(Annotation parent, AnnotationSet parse, AnnotationSet result,
    AnnotationSet origNes, AnnotationSet nes, Document doc)
{

  // System.out.println("Adding NON NPs inside "+FeatureUtils.getText(parent, text));
  AnnotationSet pContained = parse.getContained(parent);
  for (Annotation p : pContained) {
    if (isNoun(p, doc.getText()) && parent.properCovers(p) && !containsSameHead(result, p, false, doc)
        && !result.containsSpan(p) && !origNes.coversSpan(p) && !isJunk(p, doc.getText())) {
      if (!getAllTitles().contains(doc.getAnnotString(p).toLowerCase())) {// &&FeatureUtils.isCapitalized(Utils.getAnnotText(p,
                                                                        // text)))
        p = trimNP(p, parse, doc.getText());
        if (!containsSameHead(result, p, false, doc) && !result.containsSpan(p))
 ...
 */
public static void pprintMarkables(AnnotationSet basenp, String text, String dirName)
    throws IOException
{
  FileWriter fw;
...
public void run(Document doc, String[] annSetNames)
{
  ...
  // add text
  System.out.println("Adding noun phrase texts!");
  for (Annotation ourNp : bnp) {
	  String npStr = doc.getAnnotString(ourNp);
	  ourNp.setAttribute(Constants.CONLL_WORDSTR, npStr);
  }
  ...
  /*
  try {
  	pprintMarkables(bnp, text, dirName);
  }
