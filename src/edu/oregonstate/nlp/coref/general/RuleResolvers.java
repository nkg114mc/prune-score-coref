...
import edu.oregonstate.nlp.coref.features.properties.Pronoun;
...
public class RuleResolvers {
  ...
  public static void ruleResolvePronouns(HashMap<Integer,CorefChain> chains, Document doc)
  {
    // System.out.println("Rule resolving ");
    // HashMap<Annotation, ArrayList<Annotation>> posessives = new HashMap<Annotation, ArrayList<Annotation>>();
    fvm = new RuleResolvers.FeatureVectorMap();
    ArrayList<CorefChain> chainList = new ArrayList<CorefChain>(chains.values());
    Collections.sort(chainList);

    Annotation zero = new Annotation(-1, -1, -1, "zero");

    for (int i = 1; i < chainList.size(); i++) {
      CorefChain ch2 = chainList.get(i);
      if (FeatureUtils.isPronoun(ch2.getFirstCe(), doc)) {
        ruleResolvePronoun(chainList, i, doc);
      }
    }
    //System.out.println("===========================================================");
  ...
  public static void cleanUpPronounChains(HashMap<Integer,CorefChain> chains, Document doc)
  {
    for(CorefChain c:chains.values()){
      if(c.isPronoun()){
        HashMap<String,HashSet<Integer>> antes = c.getProAntecedents(c.getFirstCe());
        for(String r:antes.keySet()){
          HashSet<Integer> cc = antes.get(r);
          HashSet<Integer> newCC = new HashSet<Integer>();
          for(Integer corefId:cc){
            if(chains.containsKey(corefId)){
              CorefChain coref = chains.get(corefId);
              while(coref.getRedirect()>=0){
                coref = chains.get(coref.getRedirect());
              }
              if(!c.isAnimacyIncompatible(coref)&&!c.isGenderIncompatible(coref)
                  &&!c.isNumberIncompatible(coref)&&!c.isSemTypeIncompatible(coref)){
                newCC.add(coref.getId());
              }else{
                //System.out.println(c.toString(doc)+"\nREMOVING"+coref.toString(doc));
              }
            }
          }
          antes.put(r, newCC);
  ...
  public static Annotation ruleResolvePronoun(Annotation[] basenps, int num, Document doc)
  {
    ...
    if (FeatureUtils.getPronounPerson(str2) == PersonPronounTypeEnum.SECOND)
      ...
      for (int i = num - 1; i >= 0 && sentnum <= 3; i--) {
        ...
        if (parNum(np1, np2, doc)<=1&&!isNumberIncompatible(np1, np2, doc) && !isGenderIncompatible(np1, np2, doc)
    ...
    if (antes.size() == 1) {
      ...
      np2.setProperty(Property.RULE_NUM, "R1");
      return antes.get(0);
    }
    // Rule 2: Reflexive 
    if (reflexive) {
      ...
      for(int i=antes.size()-1; i>=0; i--){
        ...
        if(sentNum(np1, np2, doc)==0){
          if(GramRole.getValue(np1, doc).equals("SUBJECT")){
            if(DEBUG)
              System.err.println("Rule 2.1 match!!!");
            if(subjectNum!=null)
              unique=false;
      ...
      if(subject!=null&&unique){
        ...
        np2.setProperty(Property.RULE_NUM, "R2");
        return subject;
      }else{
        //the last possible antecedent
    ...
    for (Annotation np1 : antes) {
      if (sentNum(np1, np2, doc) < 2) {
        //System.out.println("Sentnum "+sentNum(np1, np2, doc)+": "+doc.getAnnotText(np1)+" -- "+doc.getAnnotText(np2));
        antes1.add(np1);
    ...
    if (antes1.size() == 1) {
      ...
      np2.setProperty(Property.RULE_NUM, "R3");
      return antes1.get(0);
    }
    ...
    if (FeatureUtils.isPossesive(str2) && antes1.size() > 0) {
      ...
      Annotation subject = null;
      Integer subjectNum = null;
      for(int i=antes1.size()-1; i>=0; i--){
        Annotation np1 = antes1.get(i);
        if(sentNum(np1, np2, doc)==0){
          if(GramRole.getValue(np1, doc).equals("SUBJECT")){
            if(DEBUG)
              System.err.println("Rule 4.1 match!!!");
            if(subjectNum!=null)
              unique=false;
            subjectNum = nums1.get(i);
            subject = antes1.get(i);
          }
        }
      }
      if(subject!=null&&unique){
        union(subjectNum.intValue(), num);
        np2.setProperty(Property.RULE_NUM, "R4");
        return subject;
      }else{
    ...
    if (antes2.size() == 1) {
      ...
      np2.setProperty(Property.RULE_NUM, "R5");
      return antes2.get(0);
    }
    ...
    while (clause != null) {
      ...
      if (antes3.size() == 1) {
        ...
        np2.setProperty(Property.RULE_NUM, "R6");
        return antes3.get(0);
      }
      ...
      for (int i = antes.size() - 1; i >= 0; i--) {
        ...
        if (sentNum(np1, np2, doc) == 1) {
          if (GramRole.getValue(np1, doc).equals("SUBJECT")) {
            // &&SyntaxUtils.isMainClause(np1, parse)){
            if (DEBUG) {
              System.err.println("Rule 7 match!!!");
            }
            if (subjectNum != null) {
              unique = false;
            }
    ...
    if (subject != null && unique) {
      union(subjectNum.intValue(), num);
      np2.setProperty(Property.RULE_NUM, "R7");
      return subject;
    }
    subjectNum = null;
    subject = null;

    // One more Rule -- the last possible antecedent
    if (antes.size()>0) {
      subjectNum = nums.get(nums.size()-1);
      subject = antes.get(antes.size()-1);
      if (subject != null) {
        union(subjectNum.intValue(), num);
        np2.setProperty(Property.RULE_NUM, "R8");
        return subject;
      }
    }
  ...
  public static HashSet<Integer> toCorefIds(HashSet<CorefChain> antes){
    HashSet<Integer> antIds = new HashSet<Integer>();
    for(CorefChain a:antes)
      antIds.add(a.getId());
    return antIds;
  }
  public static void ruleResolvePronoun(ArrayList<CorefChain> chains, int num, Document doc)
  {
    // Algorithm based on Baldwin's ('97) CogNIAC -- high precision pronoun resolution system
    CorefChain ch2 = chains.get(num);
    // Get some properties of the np
    Annotation np2 = ch2.getFirstCe();
    HashMap<String,HashSet<Integer>> ruleAntecedents=ch2.getProAntecedents(np2);
    FeatureUtils.PRTypeEnum type2 = Pronoun.getValue(np2, doc);
    String str2 = doc.getAnnotString(np2);
    boolean reflexive = FeatureUtils.isReflexive(str2);
    // Get the possible antecedents
    HashSet<CorefChain> antes = new HashSet<CorefChain>();
    ArrayList<Integer> nums = new ArrayList<Integer>();
    //DEBUG=true;
    if (DEBUG) {
      System.err.println("Pronoun: " + doc.getAnnotText(np2));
    ...
    int sentnum = 0;
    for (int i = num + 3; i >= 0 && (sentnum < 3 || (sentnum<=4&&antes.size()<=1)); i--) {
      if(i<chains.size()){
        CorefChain ch1 = chains.get(i);
        Annotation np1 = ch1.getFirstCe();
        if(!np1.equals(ch2.getFirstCe())&&ch2.getFirstCe().compareSpan(np1)>0){
          sentnum = sentNum(np1, np2, doc);
          if (DEBUG) {
            System.err.println("Possible antecedent: " + ch1.getId() + " :" + doc.getAnnotText(np1));
          }
          if ((parNum(np1, np2, doc)<2||(parNum(np1, np2, doc)<=3&&antes.size()<1))&&!ch1.isNumberIncompatible(ch2) && !ch1.isGenderIncompatible(ch2)
              && !ch1.isAnimacyIncompatible(ch2) && isWNClassComp(np1, np2, doc) && isProComp(np1, np2, doc)
              && (!Embedded.getValue(np1, doc)) && isSyntax(np1, np2, doc)&&!ch1.isSemTypeIncompatible(ch2)) {
            if (DEBUG) {
              System.err.println("Candidate antecedent: " + ch1.getId() + " :" + doc.getAnnotText(np1));
            }
            antes.add(ch1);
            ch1.addPossibleAntes(np2,ch2);
            nums.add(i);
          }
        }else if(DEBUG){
          System.err.println("Skipping: " + ch1.getId() + " :" + doc.getAnnotText(np1));          
        }
      }
    }
    //DEBUG=true;
    
    ruleAntecedents.put("1", toCorefIds(antes));
    // Rule 2: Reflexive 
    HashSet<Integer> antesR2 = new HashSet<Integer>();
    if (reflexive || FeatureUtils.isPossesive(str2)) {
      // //Look for the subject in the current sentence
      for(CorefChain ch1:antes){
        if(sentNum(ch1.getFirstCe(), np2, doc)==0){
          if(GramRole.getValue(ch1.getFirstCe(), doc).equals("SUBJECT")){
            if(DEBUG)
              System.err.println("Rule2 match:"+ch1.toString(doc));
            antesR2.add(ch1.getId());
          }
        }
      }

    }
    ruleAntecedents.put("2", antesR2);
    // Rule 3: Current + Prior
    HashSet<CorefChain> antesR3 = new HashSet<CorefChain>();
    for (CorefChain ch1 : antes) {
      if (sentNum(ch1.getFirstCe(), np2, doc) < 2) {
        if(DEBUG)
          System.err.println("Rule3 match:"+ch1.toString(doc));
        antesR3.add(ch1);
      }
    }
    ruleAntecedents.put("3", toCorefIds(antesR3));


    // Rule #5: Current sentence
    HashSet<CorefChain> antesR5 = new HashSet<CorefChain>();
    for (CorefChain ch1 : antesR3) {
      if (sentNum(ch1.getFirstCe(), np2, doc) < 1) {
        if(DEBUG)
          System.err.println("Rule5 match:"+ch1.toString(doc));
        antesR5.add(ch1);
      }
    }
    ruleAntecedents.put("5", toCorefIds(antesR5));

    // Extra Rule: Unique in the current clause (or parent clauses
    AnnotationSet parse = doc.getAnnotationSet(Constants.PARSE);
    Annotation clause = SyntaxUtils.getClause(np2, parse);
    if (clause != null) {
      HashSet<CorefChain> antesR6 = new HashSet<CorefChain>();
      for (CorefChain ch1 : antesR5) {
        if (clause.covers(ch1.getFirstCe())) {
          antesR6.add(ch1);
          if(DEBUG)
            System.err.println("Rule6 match:"+ch1.toString(doc));
        }
      }
      ruleAntecedents.put("6", toCorefIds(antesR6));
    }

    HashSet<CorefChain> antesR7 = new HashSet<CorefChain>();
    if (GramRole.getValue(np2, doc).equals("SUBJECT")) {
      // &&SyntaxUtils.isMainClause(np2, parse)){
      for (CorefChain ch1 : antes) {
        if (sentNum(ch1.getFirstCe(), np2, doc) == 1) {
          if (GramRole.getValue(ch1.getFirstCe(), doc).equals("SUBJECT")) {
            // &&SyntaxUtils.isMainClause(np1, parse)){
            if(DEBUG)
              System.err.println("Rule7 match:"+ch1.toString(doc));
            antesR7.add(ch1);
          }
        }
      }
    }
    ruleAntecedents.put("7", toCorefIds(antesR7));
    //Subject of the current sentence
    HashSet<CorefChain> antesR8 = new HashSet<CorefChain>();
    for (CorefChain ch1 : antes) {
      if (sentNum(ch1.getFirstCe(), np2, doc) == 0) {
        if (GramRole.getValue(ch1.getFirstCe(), doc).equals("SUBJECT")) {
          // &&SyntaxUtils.isMainClause(np1, parse)){
          if(DEBUG)
            System.err.println("Rule8 match:"+ch1.toString(doc));
          antesR8.add(ch1);
        }
    ...
    ruleAntecedents.put("8", toCorefIds(antesR8));
    if(DEBUG)
      System.err.println("DONE"+ch2.toString(doc)+"");
    //DEBUG=false;
  }
        ...
        if (// !FeatureUtils.isEmbedded(np1, annotations, text)
          ...
          if (hn1.equalsIgnoreCase(hn2) && compModifier(np1, np2, doc) && sentNum(np1, np2, doc) <= 2) {
            ...
            if (NPSemanticType.getValue(np1, doc).equals(NPSemanticType.getValue(np2, doc))
                ...
                && !NPSemanticType.getValue(np1, doc).equals(FeatureUtils.NPSemTypeEnum.NOTPERSON)
  ...
  private static Annotation ruleResolvePronounI(Annotation np2, Annotation[] basenps, int num, Document doc)
  {
    ...
    for (int i = num - 1; i >= 0; i--) {
      ...
      if (FeatureUtils.isPronoun(np1, doc) && FeatureUtils.getPronounPerson(doc.getAnnotText(np1)) == PersonPronounTypeEnum.FIRST
        ...
        union(i, num);
        np2.setProperty(Property.RULE_NUM, "I1");
        return np1;
      }
    }
    if (quote2 < 0) {
      ...
      for (int i = basenps.length - 1; i >= 0; i--) {
        ...
        if (type != null && type.equals(FeatureUtils.NPSemTypeEnum.PERSON)) {
          union(i, num);
          np2.setProperty(Property.RULE_NUM, "I2");
          return np1;
        }
      }
    }
      ...
      for (int i = 0; i < basenps.length; i++) {
        ...
        if (isIAntes(np1, np2, doc)) {
          union(i, num);
          np2.setProperty(Property.RULE_NUM, "I3");
          return np1;
        }
      }
    }
  ...
  private static Annotation ruleResolvePronounYou(Annotation np2, Annotation[] basenps, int num, Document doc)
  {
    ...
    for (int i = num - 1; i >= 0; i--) {
      ...
      if (FeatureUtils.isPronoun(np1, doc) && FeatureUtils.getPronounPerson(doc.getAnnotText(np1)) == PersonPronounTypeEnum.SECOND
        ...
        union(i, num);
        np2.setProperty(Property.RULE_NUM, "Y1");
        return np1;
      }
    }
    return null;
  ...
  private static Annotation ruleResolvePronounWe(Annotation np2, Annotation[] basenps, int num, Document doc)
  {
    ...
    for (int i = num - 1; i >= 0; i--) {
      ...
      if (FeatureUtils.isPronoun(np1, doc) && FeatureUtils.getPronounPerson(doc.getAnnotText(np1)) == PersonPronounTypeEnum.FIRST
        ...
        union(i, num);
        np2.setProperty(Property.RULE_NUM, "W1");
        return np1;
      }
    }
    if (quote2 < 0) {
      ...
      for (int i = 0; i < basenps.length; i++) {
        ...
        if (isWeAntes(np1, np2, doc)) {
          union(i, num);
          np2.setProperty(Property.RULE_NUM, "W2");
          return np1;
        }
      }
    }
  ...
  private static boolean isSyntax(Annotation np1, Annotation np2, Document doc)
  {
    ...
    return syntax.equals(NominalFeature.INCOMPATIBLE);
  }
  ...
  private static Integer parNum(Annotation np1, Annotation np2, Document doc)
  {
    return Integer.parseInt(Constructor.createFeature("ParNum").getValue(np1, np2, doc, fvm.getFeatureVector(np1, np2)));
  }
