...
List<CRFClassifier<ArrayCoreMap>> finders;

private Map<String, String> mTypeMap = Maps.newHashMap();

private static CRFClassifier<ArrayCoreMap> getClassifier(InputStream model)
    throws ClassCastException, IOException, ClassNotFoundException
{
  String[] args2 = {};
  Properties props = StringUtils.argsToProperties(args2);

  CRFClassifier<ArrayCoreMap> classifier = new CRFClassifier<ArrayCoreMap>(props);
  classifier.loadClassifier(model, props);
  return classifier;
}

public NamedEntityStanford(SystemConfig cfg) {
  // ////////
  String modelStr = cfg.getString("StanfordTaggerModelNames");
  System.out.println("stanford NER model names: " + modelStr);
  String[] model_names = cfg.getStringArray("StanfordTaggerModelNames");
  ...
  String[] mapping = cfg.getStringArray("semantic_type_mapping");
