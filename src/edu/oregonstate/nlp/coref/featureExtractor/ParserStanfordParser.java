...
//public static final String MODEL_NAME = "resources/Stanford/parser/englishFactored.ser.gz";
public static final String MODEL_NAME = "resources/Stanford/parser/englishPCFG.ser.gz";
...
public ParserStanfordParser() {
  try {
    ...
    lp = LexicalizedParser.loadModel(MODEL_NAME);
...
public static int addSpans(Tree parseTree, int startTokenIndx, Object[] sentToks, AnnotationSet parsesSet, Annotation parent)
{
  // System.err.println("Adding "+parseTree+" - "+startTokenIndx);
  ArrayList<Label> yield = parseTree.yield();
  ...
  int cur = parsesSet.add(((Annotation) sentToks[startTokenIndx]).getStartOffset(), ((Annotation) sentToks[len + startTokenIndx - 1]).getEndOffset(), parseTree.value(), attrs);
...
public static void addDepSpans(Collection<TypedDependency> dep, Annotation[] sentToks, AnnotationSet parsesSet)
{
  ...
  // System.out.println(d.dep().index() + " --> " + d.gov().index() + " " + d.reln().toString());
  //}
...
public void run(Document doc, String[] annSetNames)
{
  ...
  for (Annotation sentence : sentSet) {
    ...
    List<Word> stSentWord = new ArrayList<Word>();
    for (Annotation tok : toks) {
      sentWords.add(Utils.getAnnotText(tok, text));
      stSentWord.add(new Word(Utils.getAnnotText(tok, text)));
    }
    Tree ansTree = lp.apply(stSentWord);
...
public GrammaticalStructure getGramStruct(Tree ansTree) {
	GrammaticalStructure gs = gsf.newGrammaticalStructure(ansTree);
