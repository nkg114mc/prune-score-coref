...
import edu.oregonstate.nlp.coref.featureExtractor.ParagraphSplitter;
...
public class InputConllFormat implements Serializable {
	
	private static ParagraphSplitter paragraphAnnotator = new ParagraphSplitter();
	private static ParserStanfordParser stanfordParser = new ParserStanfordParser();
	
	public void inputConllFormat(ConllDocument conlldoc, boolean writeFile) {

		// extract DocPart list
		ArrayList<DocPart> dpartList = OntoNotesConllReader.constructDocParts(conlldoc.getFile().toString(), conlldoc.getDocName());
		//conlldoc.setDocPartList(dpartList);

		// construct reconcile documents
		int nPart = conlldoc.getPartNum();
		for (int i = 0; i < nPart; i++) {
			DocPart docpart = dpartList.get(i);
			String partDocName = getDocNameWithPart(conlldoc.getDocName(), i);
			Document reconDoc = constructReconcileDoc(docpart, i, partDocName);
			conlldoc.addDocument(reconDoc);
		}
	...
	public static String getDocNameWithPart(String fileName, int partID) {
		String cononical = fileName.trim();
		if (fileName.contains(".")) {
			cononical = fileName.substring(0, fileName.lastIndexOf('.')); // to remove extension
		}
		return new String(cononical + "-docpart" + partID);
	}
	...
	public Document constructReconcileDoc(DocPart docpart, int partID, String docname) {
		
		
		Document reconcileDoc = new Document(docname);

		
		// token, pos, sent
		loadOrCreateTokenPosSent(reconcileDoc, docpart);
		
		// paragraph
		loadOrCreateParagraph(reconcileDoc, docpart);
		

		// parse tree
		loadOrCreateParseTree(reconcileDoc, docpart);
		
		// ner
		loadOrCreateNer(reconcileDoc, docpart);
		
		// gold mentions
		loadOrCreateGoldMentions(reconcileDoc, docpart);
		
		// speaker
		loadOrCreateSpeaker(reconcileDoc, docpart);
		
		return reconcileDoc;
	}
	

	public void loadOrCreateTokenPosSent(Document reconcileDoc, DocPart docpart) {
		
		if (false) { // annotation set exist
			
		} else {
		
			// set text
			reconcileDoc.setText(docpart.docText);

			ArrayList<MySentence> sentList = docpart.sentList;

			AnnotationSet tokens =  new AnnotationSet(Constants.TOKEN);
			AnnotationSet pos = new AnnotationSet(Constants.POS);
			AnnotationSet sents = new AnnotationSet(Constants.SENT);
			...
			for (int i = 0; i < sentList.size(); i++) {
				...
				for (int j = 0; j < tokenList.size(); j++) {
					...
					Annotation tokenAnno = new Annotation(tline.index, tline.startOff, tline.endOff, "TOKEN");

					//System.out.println(tline.startOff + "-"+ tline.endOff + " = " + tline.token);

					tokens.add(tokenAnno);

					String tkSpan = reconcileDoc.getAnnotString(tokenAnno);
					//System.out.println("[" + tkSpan + "] == [" + tline.token + "]");
					assert (tkSpan.equals(tline.token));

					// pos
					Annotation posAnno = new Annotation(tline.index, tline.startOff, tline.endOff, tline.pos);
					pos.add(posAnno);
				}

				// for sentence
				TokenLine firstTk = tokenList.get(0);
				TokenLine lastTk = tokenList.get(tokenList.size() - 1);
				Annotation sentAnno = new Annotation(sentId, firstTk.startOff, lastTk.endOff, "SENT");
				sentAnno.setAttribute("sentNum", String.valueOf(sentId));
				sents.add(sentAnno);

				//System.out.println("===");
			}

			reconcileDoc.addAnnotationSet(tokens);
			reconcileDoc.addAnnotationSet(pos);
			reconcileDoc.addAnnotationSet(sents);
			
			//reconcileDoc.dumpAnnotationSet();

		}
	}
	
	public void loadOrCreateParagraph(Document reconcileDoc, DocPart docpart) {
		
		if (false) { // annotation set exist
			
		} else {
			paragraphAnnotator.run(reconcileDoc, Constants.PAR);
		}
	}

	public void loadOrCreateNer(Document reconcileDoc, DocPart docpart) {
		...
		if (false) { // annotation set exist
			...
			AnnotationSet ner =  new AnnotationSet(Constants.NE);
			int nerIndex = 0;

			for (int i = 0; i < sentList.size(); i++) {
				
				Stack<TokenLine> stacktkln = new Stack<TokenLine>();
				...
				for (int j = 0; j < tokenList.size(); j++) {
					...
					TokenLine tline = tokenList.get(j);
					String nerTerm = tline.ner;
					
					if (nerTerm.equals("*")) {
						// do nothing
					} else if (nerTerm.contains("(") && nerTerm.contains(")")) {
						String nerTag = extractNerTag(tline.ner);
						Annotation neAnno = new Annotation(nerIndex, tline.startOff, tline.endOff, nerTag);
						ner.add(neAnno);
						nerIndex++;
					} else if (nerTerm.contains("(") && !nerTerm.contains(")")) {
						
						stacktkln.push(tline);
						
					} else if (!nerTerm.contains("(") && nerTerm.contains(")")) {
						
						TokenLine firstTk = stacktkln.pop();
						TokenLine lastTk = tline;
						assert(firstTk.index != lastTk.index);
						String nerTag = extractNerTag(firstTk.ner);
						Annotation nerAnno = new Annotation(nerIndex, firstTk.startOff, lastTk.endOff, nerTag);
						ner.add(nerAnno);
						nerIndex++;
						
					} else {
						throw new RuntimeException("Unknown NER term: " + nerTerm);
					}

					
				}
				
				assert(stacktkln.isEmpty());
			}

			assert(ner.size() == nerIndex);
			
			reconcileDoc.addAnnotationSet(ner);
	...
	public static String extractNerTag(String conllNerTerm) {
		
		String trimNerTag = conllNerTerm.replaceAll("\\(", "")
				                        .replaceAll("\\)", "")
				                        .replaceAll("\\*", "")
				                        .trim(); // original conll type
		
		//("CARDINAL", "DATE", "EVENT", "FAC", "GPE", "LANGUAGE", "LAW", "LOC", "MONEY",
        //  "NORP", "ORDINAL", "ORG", "PERCENT", "PERSON", "PRODUCT", "QUANTITY", "TIME", "WORK_OF_ART");
		String entityType = trimNerTag; // normalized type
		...
		//System.out.println(trimNerTag);
		return entityType; //trimNerTag;
	}
	
	public void loadOrCreateGoldMentions(Document reconcileDoc, DocPart docpart) {

		if (false) { // annotation set exist

		} else {
			
			AnnotationSet goldNps =  new AnnotationSet(Constants.GS_NP);
			int goldMentIndex = 0;
			...
			for (int i = 0; i < sentList.size(); i++) {
				...
				HashMap<Integer, LinkedList<TokenLine>> cachedBegin = new HashMap<Integer, LinkedList<TokenLine>>();
				...
				for (int j = 0; j < tokenList.size(); j++) {
					...
					String corefTermAll = tline.coref;
					String[] corefTrmArr = corefTermAll.split("\\|");
					
					for (String corefTerm : corefTrmArr) {

						if (corefTerm.equals("-")) {
							// do nothing
						} else if (corefTerm.contains("(") && corefTerm.contains(")")) {
							
							int goldClId = extractCorefTag(corefTerm);
							Annotation gnpAnno = new Annotation(goldMentIndex, tline.startOff, tline.endOff, "GOLD_MENTION");
							gnpAnno.setAttribute(Constants.CLUSTER_ID, String.valueOf(goldClId));
							gnpAnno.setAttribute(Constants.CE_ID, String.valueOf(goldMentIndex));
							goldNps.add(gnpAnno);
							goldMentIndex++;
							...
							int goldClId = extractCorefTag(corefTerm);
							insertMapStack(cachedBegin, goldClId, tline);

						} else if (!corefTerm.contains("(") && corefTerm.contains(")")) {

							int goldClId = extractCorefTag(corefTerm);
							TokenLine firstTk = popMapStack(cachedBegin, goldClId);
							TokenLine lastTk = tline;
							assert(firstTk.index != lastTk.index);
							Annotation gnpAnno = new Annotation(goldMentIndex, firstTk.startOff, lastTk.endOff, "GOLD_MENTION");
							gnpAnno.setAttribute(Constants.CLUSTER_ID, String.valueOf(goldClId));
							gnpAnno.setAttribute(Constants.CE_ID, String.valueOf(goldMentIndex));
							goldNps.add(gnpAnno);
							goldMentIndex++;
							...
							throw new RuntimeException("Unknown Coref term: " + corefTerm);
						}
				...
				//for (TokenLine k : cachedBegin.values()) {
				//	System.out.println("CorefTerm: "+ k.coref);
				//}
				assert(cachedBegin.size() == 0);

			}
			assert(goldNps.size() == goldMentIndex);
			
			reconcileDoc.addAnnotationSet(goldNps);
			
			//for (Annotation gnp : goldNps) {
			//	System.out.println(reconcileDoc.getAnnotString(gnp));
			//}
	...
	public static void insertMapStack(HashMap<Integer, LinkedList<TokenLine>> cached, int goldCl, TokenLine tkln) {
		if (cached.containsKey(goldCl)) {
			LinkedList<TokenLine> lnklst = cached.get(goldCl);
			lnklst.add(tkln);
		} else {
			LinkedList<TokenLine> lnklst = new LinkedList<TokenLine>();
			lnklst.add(tkln);
			cached.put(goldCl, lnklst);
		}
	}
	
	public static TokenLine popMapStack(HashMap<Integer, LinkedList<TokenLine>> cached, int goldCl) {
		if (cached.containsKey(goldCl)) {
			LinkedList<TokenLine> lnklst = cached.get(goldCl);
			TokenLine lnk = lnklst.poll();
			if (lnklst.isEmpty()) {
				cached.remove(goldCl);
			}
			return lnk;
		} else {
	...
	public static int extractCorefTag(String conllCorefTerm) {
		
		String trimTag = conllCorefTerm.replaceAll("\\(", "")
				                      .replaceAll("\\)", "")
				                      .replaceAll("\\*", "")
				                       .trim();
		int goldClusterId = Integer.parseInt(trimTag);
		
		//System.out.println(conllCorefTerm +" --> "+ goldClusterId);
		return goldClusterId;
	}
	
	public void loadOrCreateParseTree(Document reconcileDoc, DocPart docpart) {
		
		if (false) { // annotation set exist

		} else {
			
			// load exist token set
			AnnotationSet tokens = reconcileDoc.getAnnotationSet(Constants.TOKEN);

			ArrayList<MySentence> sentList = docpart.sentList;


			
			AnnotationSet parse =  new AnnotationSet(Constants.PARSE);
			AnnotationSet dep = new AnnotationSet(Constants.DEP);
			...
			for (int i = 0; i < sentList.size(); i++) {
				...
				ArrayList<String> parseBits = new ArrayList<String>();

				ArrayList<TokenLine> tokenList = curSent.tokenList;
				for (int j = 0; j < tokenList.size(); j++) {
					TokenLine tline = tokenList.get(j);
					words.add(tline.token);
					poss.add(tline.pos);
					parseBits.add(tline.parse);
				}

				// token array
				TokenLine firstTk = tokenList.get(0);
				TokenLine lastTk = tokenList.get(tokenList.size() - 1);
				AnnotationSet tokenSent = tokens.getContained(firstTk.startOff, lastTk.endOff);
				//System.out.println(tokens.size());
				//System.out.println("(" + firstTk.startOff +","+ lastTk.endOff + ")");
				Annotation[] tokenArr = tokenSent.toArray();
				//System.out.println(tokenSent.size() + " " + tokenList.size() + " " + tokenArr.length);
				assert(tokenSent.size() == tokenList.size());
				assert(tokenArr.length == tokenSent.size());
				
				Tree<String> berkConstTree = ParseTreeUtils.assembleConstTree(words, poss, parseBits);
				edu.stanford.nlp.trees.Tree stanfConstTree = BerkeleyToStanfordTreeConverter.convert(berkConstTree);
				
				// Add constituency parse tree 
				ParserStanfordParser.addSpans(stanfConstTree, 0, tokenArr, parse, Annotation.getNullAnnot());
	
				// Add the dependencies
				GrammaticalStructure gs = stanfordParser.getGramStruct(stanfConstTree);
				Collection<TypedDependency> depStanf = gs.typedDependencies();
				ParserStanfordParser.addDepSpans(depStanf, tokenArr, dep);
				
				//System.out.println(berkConstTree.toString());

			}

			reconcileDoc.addAnnotationSet(parse);
			reconcileDoc.addAnnotationSet(dep);
	...
	public void loadOrCreateSpeaker(Document reconcileDoc, DocPart docpart) {
		
	}

	
	public static void assertAnnotationSetExist(Document doc, String annoName) {
		if (!doc.existAnnotationSet(annoName)) {
			throw new RuntimeException("Document does not contains annotation set " + annoName);
		}
	}
