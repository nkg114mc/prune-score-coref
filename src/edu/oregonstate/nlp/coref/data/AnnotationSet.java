...
public class AnnotationSet
    implements Collection<Annotation>, Serializable, Cloneable {
...
private HashMap<Integer, Annotation> annotationMapByNo = null;
...
public boolean add(Annotation A) throws ClassCastException
{
  ...
  if (annotations.containsKey(A.getId())) {
    A = new Annotation(getNextAnnotationId(), A.getStartOffset(), A.getEndOffset(), A.getType(), A.getFeatures(), A.getProperties());
  } else {
    A = A.copy();
  }
  if (annotations.containsKey(A.getId())) {
...
private void getOrderedAnnotsByNO()
{
  if (annotationMapByNo == null) {
	  int setSize = annotations.size();
	  
	  HashMap<Integer, Annotation> map = new HashMap<Integer, Annotation>();
	  List<Integer> allMenID = new ArrayList<Integer>();
	  for (Annotation ce : this) {
	    Integer mid = Integer.parseInt(ce.getAttribute(Constants.CE_ID));
	    map.put(mid, ce);
	    allMenID.add(mid);
	  }
	  
	  Collections.sort(allMenID); // sort
	  Integer menArr[] = (Integer[])(allMenID.toArray(new Integer[0]));
	  for (int i = 0; i < menArr.length; i++) {
		Integer key = menArr[i];
		Annotation ce1 = map.get(key);
		//System.out.println("Mention "+key+" normalized ID is "+i);
	  }
	  
	  // New mention ordering ======================
	  annotationMapByNo = new HashMap<Integer, Annotation>();
	  for (Annotation ce : this) {
		Integer mid = Integer.parseInt(ce.getAttribute(Constants.CE_ID));
		annotationMapByNo.put(mid, ce);
		//System.out.println("Mention "+mid);
	  }
	  // ===========================================
  }
}

public Annotation getAnnotationByNO(int annNO) 
{
   getOrderedAnnotsByNO();
   
   if (!annotationMapByNo.containsKey(new Integer(annNO))) {
	 System.out.println("No such annotation NO:" + annNO);
	 return null;
   }
   
   // new get annotation (by Chao Ma April 2013)
   Integer no = new Integer(annNO);
   Annotation anno = annotationMapByNo.get(no);
   if (anno == null) {
	   throw new RuntimeException("Annotation "+no+" does not exist!");
   }
   return anno;
}
...
{
  Annotation[] result = orderedAnnotations.toArray(new Annotation[0]);
  return result;
}
...
public AnnotationSet clone() {
    ...
    */
	try {      
       ...
       AnnotationSet obj = (AnnotationSet)oi.readObject();
       
       for (Annotation ce: obj) {
    	   
    	  int mid = ce.getId();
    	  Annotation origin_ce = this.get(mid);
    	  // directly copy
   	      //ce = origin_ce.copy();
    	  
    	  Map<Property, Object> propCopy = Maps.newHashMap();
    	  Map<Property, Object> allProp = origin_ce.getProperties();
		  for (Property prop : allProp.keySet()) {
		    Object propObj = origin_ce.getProperties().get(prop);
		    propCopy.put(prop, propObj);
    	  }
		  ce.setProperties(allProp);
