  ...
  for (Annotation a : nps.getOrderedAnnots()) {
    ...
    switch (matchStyle) {
      ...
      case UW:
        ...
        break;
      case ONTO:
      	//match = matchAnnotationOntoStyle(a, gsNps, doc);
    	match = newMatchAnnotationOntoStyle(a, gsNps, doc);
        break;
      default:
        match = matchAnnotationACEStyle(a, gsNps, doc);
        break;
...
public static Annotation matchAnnotationOntoStyle(Annotation a, AnnotationSet gsNps, Document doc)
{
  Annotation match = null;
  AnnotationSet overlapCoref = gsNps.getOverlapping(a);
  ...
  if (overlapCoref != null) {
    ...
    for (Annotation cur : overlapCoref) {
      ...
      //Now try to find a head match
      Annotation curHead = HeadNoun.getValue(cur, doc);      
      //System.out.println(cur);
      if (curHead != null ) {
        int headStart = curHead.getStartOffset();
        int headEnd = curHead.getEndOffset();
        ...
        //if(!headMatch){
        //	System.out.println("ANNOT: "+doc.getAnnotText(a)+"  HEAD:"+head);
        //}
        // if(FeatureUtils.isProperName(np, annotations, text)){
        // Annotation pn = (Annotation)FeatureUtils.LINKED_PROPER_NAME.getValue(np, annotations, text);
        // if(pn!=null&&pn.getStartOffset()==headStart&&pn.getEndOffset()==headEnd)
...
public static Annotation newMatchAnnotationOntoStyle(Annotation a, AnnotationSet gsNps, Document doc)
{
	  ...
	  */
/*    
 no head match currently (2014 - 4- 22)
