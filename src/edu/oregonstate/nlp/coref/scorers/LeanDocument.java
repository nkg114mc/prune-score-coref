...
public TreeMap<Integer,Integer> joinClusters(Integer toClId, Integer fromClId){
  TreeMap<Integer,Integer> fromCluster = clusters.get(fromClId);
  TreeMap<Integer,Integer> toCluster = clusters.get(toClId);
//  System.out.println("Joining "+toClId+" and "+fromClId);
//  for(Integer key:fromCluster.keySet()){
//    System.out.println(key+": "+fromCluster.get(key));
//  }

  for(Integer item:fromCluster.keySet()){
    NPchains.put(item, toClId);
    toCluster.put(item, item);
  }
  numChains--;
  clusters.remove(fromClId);
//  System.out.println(this);
  return fromCluster;
}
public void restoreCluster(TreeMap<Integer,Integer> cl, Integer fromId, Integer clId){
//  System.out.println("Restore "+clId);
//  for(Integer key:cl.keySet()){
//    System.out.println(key+": "+cl.get(key));
//  }
  TreeMap<Integer,Integer> inCluster = clusters.get(fromId);
  
  for(Integer item:cl.keySet()){
    NPchains.put(item, clId);
    inCluster.remove(item);
  }
  clusters.put(clId, cl);
  numChains++;
//  System.out.println(this);
