...
/*
  ...
  HashMap<Integer, Integer> mymatch = key.matches;
  for (Integer mid : mymatch.keySet()) {
	 Integer matchId = mymatch.get(mid);
	 System.out.println("key match: "+ mid+" - "+matchId);
  }
  mymatch = response.matches;
  for (Integer mid : mymatch.keySet()) {
	Integer matchId = mymatch.get(mid);
	System.out.println("response match: "+ mid+" - "+matchId);
  }
*/
  ...
  while (goldChains.hasNext()) {
    ...
    while (nouns.hasNext()) {
      ...
      if (twin != null) {
    	...
    	Integer twinCID = response.getClusterNum(twin);
        TreeMap<Integer, Integer> responseChain = response.getChain(twinCID);
        //System.out.println("twin = "+twin+" response clustId "+response.getClusterNum(twin));
        if (responseChain == null) {
          //System.out.println("Key:\n" + key + "\n*************************\nResponse:\n" + response);
          responseChain = new TreeMap<Integer, Integer>();
          responseChain.clear();
          //throw new RuntimeException("null response for " + entity + " twin " + twin + " twinClusterID " + twinCID);
        }
